...
// --- REFINE HIGH-CONTRAST DESIGN SYSTEM ---
const styles = {
  ...
  // Large Interactive Modal additions
...
const POReceives = () => {
    ...
    // Live tracking of uploaded temporary assets on the cluster filesystem
    ...
    // Zoom and pan tracking states
    const [drZoom, setDrZoom] = useState(1);
    ...
    const generateNewBatchKey = useCallback(() => {
        const timestamp = Date.now();
        const randomStr = Math.random().toString(36).substring(2, 7).toUpperCase();
        setBatchReference(`PO-BATCH-${timestamp}-${randomStr}`);
        setStagingStatus({ po_attachment: false, dr_attachment: false });
    }, []);

    // RESTORED ORIGINAL PATH 1 ENDPOINT STRUCTURE
    const fetchPoHistory = useCallback(async () => {
        try {
            const res = await fetch('https://api.ricalgen.eu.org/api/po-receives/history');
            if (res.ok) {
                const data = await res.json();
                setPoHistory(data);
            }
        } catch (err) {
            console.error("Failed to sync historical ledger:", err);
        }
    }, []);
    ...
    const checkStagingStatus = useCallback(async () => {
        ...
        try {
            const res = await fetch(`https://api.ricalgen.eu.org/api/po-receives/check-staging/${batchReference}`);
            if (res.ok) {
                const data = await res.json();
                setStagingStatus({
            ...
            console.error("Staging checker validation drop:", err);
        }
    }, [batchReference]);
    ...
    useEffect(() => {
        ...
        const fetchClients = async () => {
            try {
                const res = await fetch('https://api.ricalgen.eu.org/api/po-receives/clients');
                if (!res.ok) throw new Error('Failed to load corporate accounts logs');
                const data = await res.json();
                setClients(data);
            } catch (err) {
                showNotice('error', `Dropdown Hydration Error: ${err.message}`);
            }
        };
        ...
        const fetchServerInfo = async () => {
            try {
                const res = await fetch('https://api.ricalgen.eu.org/api/server-info');
                const data = await res.json();
                if (data.ip) setServerIp(data.ip);
            } catch (err) {
                console.error("Local context fallback routing configured:", err);
            }
        };
        ...
        fetchServerInfo();
        fetchPoHistory();
    }, [generateNewBatchKey, fetchPoHistory, showNotice]);

    // Active polling rule to scan pipeline files
    useEffect(() => {
        let interval = null;
        if (batchReference) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // RESTORED ORIGINAL PATH 1 ENDPOINT STRUCTURE
            const res = await fetch('https://api.ricalgen.eu.org/api/po-receives', {
                method: 'POST',
            ...
            if (!res.ok || result.success === false) {
                throw new Error(result.error || 'Failed to register configuration matrix.');
            }

            showNotice('success', `Purchase Order successfully registered under reference sequence: ${batchReference}`);
            setFormData({
                customer_id: '',
                amount: '',
                po_date: new Date().toISOString().split('T')[0],
    ...
    // Zoom Adjustment Modifiers
    const adjustDrZoom = (factor) => {
        setDrZoom(prev => {
            const next = Math.min(Math.max(prev + factor, 1), 5);
            if (next === 1) setDrPan({ x: 0, y: 0 });
            return next;
        });
    };
    
    const adjustPoZoom = (factor) => {
        setPoZoom(prev => {
            const next = Math.min(Math.max(prev + factor, 1), 5);
            if (next === 1) setPoPan({ x: 0, y: 0 });
            return next;
        });
    };

    // Drag-And-Pan Capture Enclosure (DR)
    ...
    // Drag-And-Pan Capture Enclosure (PO)
    ...
    // VERSION 2 UPLOAD SCANNER URL
    const currentMobileScannerUrl = batchReference 
        ? `https://api.ricalgen.eu.org/mobile-uploads2/${batchReference}`
        : '';
    ...
    return (
        <div style={styles.container}>
            ...
            <div className="po-master-grid" style={styles.layoutGrid}>
                    ...
                    <form onSubmit={handleSubmit} style={styles.form}>
                        ...
                        <div style={styles.formRow}>
                            ...
                            <div style={styles.formGroup}>
                                ...
                                >
                                    ...
                                    <option value="Terms-15">Terms 15 Days</option>
                                    <option value="Terms-30">Terms 30 Days</option>
                                    <option value="Terms-60">Terms 60 Days</option>
                                    <option value="Dated-Check">Post-Dated Check</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div style={styles.formGroup}>
                            ...
                            <select 
                                ...
                                style={styles.select}
                            >
                                <option value="pending">Pending Review / Unserved</option>
                                <option value="served">Served / Stock Transferred</option>
                            </select>
                        </div>
                ...
                {/* DASHBOARD SCANNER ENGINE BLOCK */}
                <div className="po-qr-container" style={{ ...styles.card, ...styles.scannerCard }}>
                    ...
                    <div style={styles.qrContainer}>
                        {currentMobileScannerUrl ? (
                            <QRCodeSVG value={currentMobileScannerUrl} size={140} bgColor={"#13151a"} fgColor={"#00ff88"} level={"M"} includeMargin={false} />
                        ) : (
                    ...
                    <a 
                        href={currentMobileScannerUrl || '#'}
                        target={currentMobileScannerUrl ? "_blank" : "_self"} 
                        rel="noopener noreferrer" 
                        style={{ ...styles.redirectBtn, ...(!currentMobileScannerUrl ? styles.redirectBtnDisabled : {}) }}
                        onClick={(e) => !currentMobileScannerUrl && e.preventDefault()}
                    >
                        📸 OPEN CAMERA LINK
                    </a>

                    {/* LIVE STAGING UPLOAD MONITOR MONITOR PIPELINE */}
            ...
            {/* LEDGER HISTORICAL OUTPUT VIEWER GRID */}
            <div style={styles.tableSection}>
                <div style={styles.tableHeaderContainer}>
                    <h3 style={styles.tableTitle}>Registered Purchase Orders Ledger</h3>
                    <button onClick={fetchPoHistory} style={styles.refreshBtn}>🔄 Sync Logs</button>
                </div>
                                    ...
                                    <tr key={row.batch_reference || idx} style={styles.tableRow}>
                                        ...
                                        <td style={styles.td}>
                                            {row.po_attachment || row.dr_attachment ? (
                                                <button type="button" onClick={() => setViewDocsRow(row)} style={styles.viewAttachmentBtn}>👁️ View Docs</button>
                                            ) : (
                                                <span style={styles.mutedText}>—</span>
                                            )}
                                        </td>
                                        <td style={styles.tdRight}>
                                            <button onClick={() => setActiveInspectionBatch(row.batch_reference)} style={styles.rowAttachmentBtn}>
            ...
            {/* DYNAMIC DOCUMENT ATTACHMENT MODAL BRIDGE */}
            {activeInspectionBatch && (
                <div style={styles.modalOverlay}>
                    <div style={styles.modalContent}>
                        <div style={styles.modalHeader}>
                            <h4 style={styles.modalTitle}>DOCUMENT ATTACHMENT BRIDGE</h4>
                            <button style={styles.modalCloseBtn} onClick={() => setActiveInspectionBatch(null)}>✕</button>
                        </div>
                        <div style={styles.modalBody}>
                            ...
                            <div style={styles.modalQrWrapper}>
                                <QRCodeSVG value={`https://api.ricalgen.eu.org/mobile-uploads/${activeInspectionBatch}`} size={160} bgColor={"#111827"} fgColor={"#00ff88"} level={"M"} includeMargin={false} />
                            </div>
                            ...
                            <a href={`https://api.ricalgen.eu.org/mobile-uploads/${activeInspectionBatch}`} target="_blank" rel="noopener noreferrer" style={styles.modalActionLink}>
            ...
            {/* INTERACTIVE DRAG-PAN SCANNING VIEWER CANVAS */}
            {viewDocsRow && (
                <div style={styles.modalOverlay} onClick={() => setViewDocsRow(null)}>
                    <div className="po-large-modal-content" style={styles.largeModalContent} onClick={(e) => e.stopPropagation()}>
                        <div style={styles.modalHeader}>
                            ...
                            <button style={styles.modalCloseBtn} onClick={() => setViewDocsRow(null)}>✕</button>
                        </div>
                        
                        <div className="po-docs-viewer-grid" style={styles.documentViewerGrid}>
                            {/* DELIVERY RECEIPT FRAME BLOCK */}
                            <div className="po-doc-frame" style={styles.documentFrame}>
                                <div style={styles.docFrameHeader}>
                                    <h5 style={styles.docFrameTitle}>DR ATTACHMENT (Delivery Receipt)</h5>
                                    {viewDocsRow.dr_attachment && (
                                        <div style={styles.zoomControls}>
                                            <button style={styles.zoomBtn} onClick={() => adjustDrZoom(-0.25)}>—</button>
                                            <span style={styles.zoomIndicator}>{Math.round(drZoom * 100)}%</span>
                                            <button style={styles.zoomBtn} onClick={() => adjustDrZoom(0.25)}>+</button>
                                            <button style={styles.zoomBtn} onClick={() => { setDrZoom(1); setDrPan({x:0, y:0}); }}>↺</button>
                                        </div>
                                    )}
                                </div>
                                <div 
                                    className={drZoom > 1 ? "pan-canvas-grab" : ""}
                                    style={styles.imageCanvas}
                                    onMouseDown={(e) => startDrDrag(e.clientX, e.clientY)}
                                    onMouseMove={(e) => moveDrDrag(e.clientX, e.clientY)}
                                    onMouseUp={() => setIsDraggingDr(false)}
                                    onMouseLeave={() => setIsDraggingDr(false)}
                                    onTouchStart={(e) => e.touches.length === 1 && startDrDrag(e.touches[0].clientX, e.touches[0].clientY)}
                                    onTouchMove={(e) => e.touches.length === 1 && moveDrDrag(e.touches[0].clientX, e.touches[0].clientY)}
                                    onTouchEnd={() => setIsDraggingDr(false)}
                                >
                                    {viewDocsRow.dr_attachment ? (
                                        <img 
                                            src={`https://api.ricalgen.eu.org${viewDocsRow.dr_attachment}`} 
                                            alt="Delivery Receipt Frame" 
                                            draggable="false"
                                            style={{ 
                                                ...styles.embeddedDocImg, 
                                                transform: `translate(${drPan.x}px, ${drPan.y}px) scale(${drZoom})` 
                                            }}
                                            onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'block'; }}
                                        />
                                    ) : null}
                                    <div style={{...styles.imgFallbackText, display: viewDocsRow.dr_attachment ? 'none' : 'block'}}>
                                        ⚠️ No Digital Delivery Receipt File Parsed
                                    </div>
                                </div>
                            </div>

                            {/* PURCHASE ORDER FRAME BLOCK */}
                            <div className="po-doc-frame" style={styles.documentFrame}>
                                <div style={styles.docFrameHeader}>
                                    <h5 style={styles.docFrameTitle}>PO ATTACHMENT (Purchase Order)</h5>
                                    {viewDocsRow.po_attachment && (
                                        <div style={styles.zoomControls}>
                                            <button style={styles.zoomBtn} onClick={() => adjustPoZoom(-0.25)}>—</button>
                                            <span style={styles.zoomIndicator}>{Math.round(poZoom * 100)}%</span>
                                            <button style={styles.zoomBtn} onClick={() => adjustPoZoom(0.25)}>+</button>
                                            <button style={styles.zoomBtn} onClick={() => { setPoZoom(1); setPoPan({x:0, y:0}); }}>↺</button>
                                        </div>
                                    )}
                                </div>
                                <div 
                                    className={poZoom > 1 ? "pan-canvas-grab" : ""}
                                    style={styles.imageCanvas}
                                    onMouseDown={(e) => startPoDrag(e.clientX, e.clientY)}
                                    onMouseMove={(e) => movePoDrag(e.clientX, e.clientY)}
                                    onMouseUp={() => setIsDraggingPo(false)}
                                    onMouseLeave={() => setIsDraggingPo(false)}
                                    onTouchStart={(e) => e.touches.length === 1 && startPoDrag(e.touches[0].clientX, e.touches[0].clientY)}
                                    onTouchMove={(e) => e.touches.length === 1 && movePoDrag(e.touches[0].clientX, e.touches[0].clientY)}
                                    onTouchEnd={() => setIsDraggingPo(false)}
                                >
                                    {viewDocsRow.po_attachment ? (
                                        <img 
                                            src={`https://api.ricalgen.eu.org${viewDocsRow.po_attachment}`} 
                                            alt="Purchase Order Frame" 
                                            draggable="false"
                                            style={{ 
                                                ...styles.embeddedDocImg, 
                                                transform: `translate(${poPan.x}px, ${poPan.y}px) scale(${poZoom})` 
                                            }}
                                            onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'block'; }}
                                        />
                                    ) : null}
                                    <div style={{...styles.imgFallbackText, display: viewDocsRow.po_attachment ? 'none' : 'block'}}>
                                        ⚠️ No Digital Purchase Order File Parsed
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={styles.modalFooter}>
                            ...
                            <button style={styles.closeActionBtn} onClick={() => setViewDocsRow(null)}>Dismiss Canvas View</button>
                        </div>
                    </div>
                </div>
            )}
